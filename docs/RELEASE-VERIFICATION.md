...
| `npm test` | Passed: 43 tests in 10 files |
...
| WebKit 26.6 on hosted Ubuntu CI | Pending this candidate's CI run; earlier foundation run passed the engine |
| WebKit 26.6 in this local container | Cannot launch: required GTK/GStreamer/libopus/libsoup libraries are absent |

The browser suite checks asset loading and transparency, manifest frame bounds,
movement input, world/camera preview, interaction preview, title/start/pause
screen flow, mute/unavailable-audio behavior, and audio effect lifecycle. It does
